...
export function getRecipesForSkill(skill: 'smithing' | 'fletching'): Recipe[] {
...
export function hasIngredients(
  ...
  return ingredients.every(
    (ing) => countItemInInventory(inventory, ing.itemId) >= ing.quantity
  );
}
...
export function getMaxCraftableCount(
  recipe: Recipe,
  inventory: Item[]
): number {
  if (!hasIngredients(inventory, recipe.ingredients)) return 0;
  let max = Infinity;
